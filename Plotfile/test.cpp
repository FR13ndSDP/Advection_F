...
// if slice from origin
bool origin = false;

// location of point on slice
Array<Real,3> location = {std::numeric_limits<Real>::lowest(),
                            std::numeric_limits<Real>::lowest(),
                            std::numeric_limits<Real>::lowest()};

// default variable to draw
std::string compname = "density";

// max level used on slice
int max_level = -1;
...
void readAndWrite()
{
    // get the palette, write to color arrays
    Array<unsigned char,256> r, g, b, a;
    int numElements = 0;
    numElements = loadPalette("Palette", r, g, b, a);
    ...
    // handle if the var_name is correct or given
    ...
    // get the index of origin in domain
    // location not given, default to center
    ...
    // any of the location is given, so should not be -1.e36
    if (location[0] > -1.e36 || location[1] > -1.e36 || location[2] > -1.e36) {
        ...
        for (int idim = 0; idim < dim; ++idim) {
            // get the index in domain
            iloc[idim] = static_cast<int>((location[idim]-problo[idim]) / dx[idim]);
        }
    }

    // limit the location to avoid "out of range"
